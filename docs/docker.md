...
코드 수정 없이 이미 빌드된 Docker 이미지를 사용하려면 다음 명령어를 사용할 수 있습니다.

```sh
docker compose pull
```

다운로드에서 서버 구동까지 한번에 하려면 다음 명령어로도 가능합니다.

```sh
docker compose up --wait --pull
```
...
```sh
docker compose --profile prod up --wait --build
```
